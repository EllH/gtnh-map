...
overviewer.util = {
    ...
    'fromWorldToLatLng': function(x, y, z, tset) {
        ...
        // World <-> map maths runs on Overviewer's 24px-per-block grid. A tileset
        // drawn at a higher resolution has extraZoom more levels on top of it
        // (gtnh/tools/build_viewer.py), which must not stretch the world.
        var zoomLevels = tset.zoomLevels - (tset.extraZoom || 0);
    ...
    'fromLatLngToWorld': function(lat, lng, tset) {
        ...
        // A map pixel is a whole line of world points, so one height has to be
        // assumed. Overviewer assumes 64; a tileset may name its own ground level
        // (gtnh/tools/build_viewer.py does, for dimensions whose ground is at 128).
        var refY = (tset.coordsY !== undefined) ? tset.coordsY : 64;
        var point = Array();
        point.x = 0;
        point.y = refY;
        point.z = 0;
